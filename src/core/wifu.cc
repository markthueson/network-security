...
void setup_nfq(){
    ...
    handle_ = nfq_open();
    if (!handle_) {
        perror("Error: Can't get netfilter handle");
        exit(1);
    }
    ...
    if (nfq_bind_pf(handle_, AF_INET) < 0) {
        perror("Error: Can't bind netfilter handle");
        exit(1);
    }
    ...
    if (!qh) {
        perror("Error: Can't create netfilter input queue");
        exit(1);
    }
    if (nfq_set_mode(qh, NFQNL_COPY_PACKET, 0xffff) < 0) {
        perror("Error: Can't set netfilter copy mode for input queue");
        exit(1);
    }
    queue_handle_[0] = qh;
    ...
    if (!qh) {
        perror("Error: Can't create netfilter output queue");
        exit(1);
    }
    if (nfq_set_mode(qh, NFQNL_COPY_PACKET, 0xffff) < 0) {
        perror("Error: Can't set netfilter copy mode for output queue");
        exit(1);
    }
    queue_handle_[1] = qh;
    ...
    if (!qh) {
        perror("Error: Can't create netfilter forward queue");
        exit(1);
    }
    if (nfq_set_mode(qh, NFQNL_COPY_PACKET, 0xffff) < 0) {
        perror("Error: Can't set netfilter copy mode for forward queue");
        exit(1);
    }
    queue_handle_[2] = qh;
    ...
    if (!qh) {
        perror("Error: Can't create netfilter prerouting queue");
        exit(1);
    }
    if (nfq_set_mode(qh, NFQNL_COPY_PACKET, 0xffff) < 0) {
        perror("Error: Can't set netfilter copy mode for prerouting queue");
        exit(1);
    }
    queue_handle_[3] = qh;
    ...
    if (!qh) {
        perror("Error: Can't create netfilter postrouting queue");
        exit(1);
    }
    if (nfq_set_mode(qh, NFQNL_COPY_PACKET, 0xffff) < 0) {
        perror("Error: Can't set netfilter copy mode for postrouting queue");
        exit(1);
    }
    queue_handle_[4] = qh;
}
...
int main(int argc, char **argv) {
    ...
    if(verbose_) cout << "Setting up logger" << endl;
    setup_logger();
    if(verbose_) cout << "Setting up iptables" << endl;
    setup_iptables(true);
    if(verbose_) cout << "Setting up net filter queue" << endl;
    setup_nfq();
    fd = nfq_fd(handle_);

    if(verbose_) cout << "Receiving" << endl;
